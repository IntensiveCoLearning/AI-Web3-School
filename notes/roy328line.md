...
Demo Day | Hackathon 項目展示 + 四週學習總回顧

今日下午1:00 Demo Day 正式舉行，展示 AI Security（Defense-in-Depth）項目：Prompt Injection 防禦 + Tool Call 白名單 + Pact 範圍約束 + EAS 鏈上 attestation 審計記錄。Q&A 聚焦攻擊向量覆蓋完整性、鏈上 gas 成本與 Web2 安全工具的差異。觀摩其他項目：Cobo CAW Agentic Commerce 閉環、GLM-5.1 合約審查助手、多簽治理輔助工具，三者均清晰回答「為什麼同時需要 AI 和 Web3」。四週學習核心收獲：AI Agent 動作可自動化邊界 + 人工確認節點設計 + 可驗證鏈上記錄三者結合，構成 AI × Web3 可信執行的基礎框架。
<!-- DAILY_CHECKIN_2026-06-14_END -->

# 2026-06-12
<!-- DAILY_CHECKIN_2026-06-12_START -->

Week 4 最終衝刺 | Hackathon 最終提交答疑 + 例會 + Final Submission Pack 整備

今日參加 Co-learning Hackathon 最終提交答疑，核心討論圍繞 Demo 跑通標準、驗證材料收集和 README 呈現方式。完成 Final Submission Pack 最後整備：GitHub repo 結構確認、Demo 可運行性驗證、鏈上 EAS attestation 記錄確認（Base Sepolia）。Week 4 例會討論 Demo Day 格式（6/14，3分鐘展示 + 2分鐘Q&A）和下週課程預告。

核心學習：在有限時間內做出可驗證最小有意義成果的方法論——先凍結範圍、再執行主流程、再收集驗證材料、最後包裝敘事。AI Security Demo（Defense-in-Depth 四層架構）完成 Final Submission Pack 整備。
...
Hackathon Demo 反思：AI Security Demo 完成了 Defense-in-Depth 四層架構的最小可驗證原型。反思三個可改進點：攻擊向量測試集多樣性不足、鏈上 attestation 應擴展到所有決策點、需要 API 介面讓其他系統整合安全層。

Dev Tooling 探索：AI 輔助代碼審查工具需要防止 Prompt Injection（開發者可能在代碼注釋中嵌入惡意指令）。這是 AI Security 在 Dev Tooling 場景的具體應用，也是 Hackathon 項目從「執行時安全防護」擴展到「開發時安全審查」的自然方向。

學習地圖回顧：完整回顧 AI × Web3 School 四週學習路徑，確認知識點連接：AI 基礎 + Web3 基礎 + AI × Web3 Bridge + 前沿探索（AI Security Hackathon Demo）。PR 連結：https://github.com/IntensiveCoLearning/AI-Web3-School/pull/54
...
1.  Governance AI 核心概念：AI 輔助 DAO 治理的應用場景（提案摘要、投票分析、參與度提升），以及 AI 建議固化風險與可審計性設計。
    
2.  2\. Hackathon Demo CLI 輸出優化：顏色編碼決策結果（ALLOW/DENY/ESCALATE），提升 Defense-in-Depth 架構的可視化展示效果。
    
3.  3\. Hackathon 提交材料清單確認：Demo 代碼、測試套件、README、鏈上 attestation 記錄與 Demo 影片腳本草稿。
    
4.  4\. Week 4 學習地圖回顧：AI Security + AI Privacy + Governance AI 三者共同構成 AI Agent 在 Web3 場景可信運行的基礎設施。
...
Week 4 Hackathon Build Day 4 | MVP 核心代碼實作完成 + Verifiable AI 概念深化

今日完成 AI Security Demo 的兩個核心模組：Prompt Injection 檢測器（10 種攻擊向量攔截率 100%）與 Tool Call Validator（三層驗證邏輯）。審計日誌接入 Base Sepolia EAS 完成第一筆鏈上 attestation。深化理解 Verifiable AI 核心：鏈上 attestation 讓 Agent 的每個拒絕決策可被外部驗證，是 DeFi Agent 信任基礎設施的關鍵。整理 README 初稿，完成 Demo 故事線。
...
**Tool Use**：讓 Agent 從「會回答」變成「能做事」。工具設計六維度：輸入 schema / 權限範圍 / 是否只讀 / 外部副作用 / 記錄方式 / 人工確認觸發條件。一個能「讀網頁」和能「寫入系統」的 Agent 被惡意網頁影響，後果完全不同。Tool Use 的權限分級比工具能力本身更重要。

**Planning**：模型生成的計劃是**候選路線，不是授權**。越靠近高風險動作，計劃越需要被系統規則拆開逐步檢查，而不是一次批准整條計劃鏈。

**State**：生產系統需要可查詢、可恢復、可審計的外置 State。State 應記錄：環境（鏈 ID、區塊高度）、版本、工具調用結果、確認請求、撤銷事件。

**Reflection**：自我檢查可以提高質量，確定性檢查才能承載風險。Reflection 不能替代外部驗證 + 人工確認。
