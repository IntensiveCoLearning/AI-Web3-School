...
HireNet Hackathon 答辩准备。Cobo API 调研 → 改用 Mock 骨架展示。完成 Demo 配音脚本（三端视角+盈利模型）、README 重写、前端修复 3 处。项目 push GitHub，480 tests。
...
HireNet Hackathon Day 4-5：后端全线贯通。

Phase 2 收口（多方分账 + JWT auth，378 tests），Phase 3 Cobo 真接入（Ed25519 签名 + 异步结算 + settling→settled 状态机，480 tests）。Demo 全流程可以跑了——企业付钱→链上结算→创作者收钱，tx\_hash 可查。三端 15 页前端全部到位。
...
Hackathon：HireNet 产品设计冲刺

今天没写代码，但把 HireNet 从「能跑的原型」推进到了「有清晰产品方向的 Hackathon 项目」。

MVP 范围锁定：企业描述需求 → AI 分析任务 → 匹配 Agent → Cobo Pact 支付 → 版税记账。10 个页面 UX 流程全部画完，企业端和创作者端从角色选择页分叉，两条完全不同的路径。

设计系统定调：决定走莫兰迪低饱和色系。主色鼠尾草绿、背景暖米白、财务数字烟粉。整个产品的气质是「低压力、克制、不像 AI 产品」——因为用户带着焦虑来（招人/找工作的压力），界面不该再施加一层压力。

赞助方对齐：Cobo 在 Pact 确认页展示 MPC 托管+手机审批流程，智谱 GLM-4 驱动所有 LLM 调用。Demo 脚本已写好，3 分钟覆盖完整闭环。

技术决策：后端零重构（只换模型+加端点），前端 React 重写（声明式 UI 架构），Chat 边界收紧（只在追问环节用对话）。

行业洞察：看了 Ruben Casas 的生成式 UI 演讲。「声明式 UI 是当下的最优解」这个结论直接指导了前端架构——AI 描述布局，Design System 保证一致性。
