...
### 1\. cobo-agent-autopay：AI Agent 的自动支付授权管理层

-   定位：给 Agent 做的「花钱权限管家」，让 Agent 能自动支付又不乱花钱。
    
-   技术栈：Base 主网 + USDC + x402 支付协议 + Cobo CAW 钱包
    
-   核心逻辑：用「Treasury Agent（管钱）」和「Spending Agent（花钱）」双 Agent 模式，实现**零人工干预自动充值**。
    
-   理解：就像给 Agent 办了一张带额度限制的自动充值副卡，Agent 需要付费时自动扣款，不用每次等你手动确认。
    

### 2\. NFT 如何破解实体资产困境：葡萄酒资产预售案例

-   传统模式痛点：葡萄酒从酿造到陈酿要几年，酒庄资金全被原料、人工、仓储压死，没法提前变现，抗风险能力弱。
    
-   NFT 解决方案：把未来批次的酒液、酒桶份额铸造成链上 NFT，作为「数字酒票」提前预售，锁定未来价值，立刻回笼资金。
    
-   延伸参考：茅台的数字化尝试（防伪溯源、数字藏品），但还没打通「确权 - 流转 - 交易」的完整闭环，而这类资产预售 NFT 可以承载更深层的金融属性。
    
-   理解：就像把 “未来的葡萄酒” 做成可交易的数字门票，提前卖出去，酒庄不用再等几年才能拿到钱。
<!-- DAILY_CHECKIN_2026-06-14_END -->

# 2026-06-13
<!-- DAILY_CHECKIN_2026-06-13_START -->

已提交项目的团队原则上都可以参与 Demo展示。由于本次提交项目数量较多，DemoDay整体时间会比较长，也请各位团队提前预留好时间。

目前Demo Day暂定于北京时间6月14日13:00开始，开场后会分为三个Demo场次，中间安排两次休息:

第一场:第1-26个项目

中场休息:10-15分钟

第二场:第27-52个项目中场休息:10-15分钟第三场:第53-77个项目
...
### 1\. 接入预测下注（Prediction Betting）类 DApp，怎么用 CAW？

-   **核心结论**：CAW 本身支持签名和合约调用，具体方式要看 DApp 要求。
    
    -   像 Polymarket 这类预测市场，直接用 **EIP-712 签名** 就能接入，不用复杂合约。
        
-   **小白理解**：Agent 下注本质就是给交易签名，CAW 支持签名就够用了，具体按目标 DApp 的文档来就行。
    

### 2\. 机构 vs 散户的不同需求，CAW 怎么平衡？

-   **核心结论**：产品本身不做偏向，把选择权交给用户，通过 **Pact 的粒度** 来平衡：
    
    -   散户用：粗粒度 Pact（简单、方便、一键授权）
        
    -   机构用：细粒度 Pact（强控制、强风控，每笔交易都有限制）
        
-   **理解**：就像给钱包装不同权限锁，你要简单就用 “密码锁”，要安全就用 “指纹 + 密码双锁”。
    

### 3\. 跨链 DEX、借贷这类复杂 DeFi 策略，CAW 能处理吗？

-   **核心结论**：CAW 在 API 层面支持 **EIP-712 子字段限制** 和 **合约调用参数级限制**，可以对 ERC20 Approve、Permit、Swap、Deposit 这些操作做参数级风控。
    
-   **小白理解**：Agent 做复杂交易时，CAW 能在 API 层给交易加 “安全过滤”，比如限制 Swap 的滑点、借贷的额度，防止乱操作。
    

### 4\. Pact 里的 intent、rule、completion condition 是什么？

-   **核心结论**：直接看官方文档：[https://www.cobo.com/products/agentic-wallet/manual/start-here/what-is-a-pact](https://www.cobo.com/products/agentic-wallet/manual/start-here/what-is-a-pact)
    
-   **小白理解**：
    
    -   `intent`：Agent 要做什么事（比如 “买一个商品”）
        
    -   `rule`：做事的规则（比如 “只能付 1 次款，金额不超过 100USDC”）
        
    -   `completion condition`：完成条件（比如 “交易上链成功就结束”）
        

### 5\. Cobo 在 AI Agent 支付和机器经济的长期战略？

-   **核心结论**：目前还在探索阶段，大方向是 **平衡安全与易用性**，核心竞争力就是让 Agent 花钱既安全又方便。
    
-   **理解**：简单说就是 “不让 Agent 乱花钱，又不让用户操作太麻烦”，这也是 CAW 最核心的价值。
    

### 6\. 单次授权购买意图的最小安全 Policy 怎么设计？（Cat-Agent 项目）

-   **核心结论**：按业务场景尽量收敛权限，Policy 定义参考文档：[https://www.cobo.com/products/agentic-wallet/manual/reference/pact-policies](https://www.cobo.com/products/agentic-wallet/manual/reference/pact-policies)
    
-   **关键要点**：
    
    -   必须显式声明：链、源地址、目标合约、参数、金额等
        
    -   危险默认设置：开放式权限、无上限、无过期时间的 Policy
        
    -   expiry/revocation 设计：一定要加过期时间，单次授权只允许一笔交易，用完自动失效，支持用户随时撤销。
        

### 7\. 哪些风控放在钱包 Policy 层，哪些放在 App 层？

-   **核心结论**：看用户和 CAW 的接触方式：
    
    -   用户直接控制 CAW → 边界放在钱包 Policy 层
        
    -   用户只能通过 App 间接用 CAW → 边界放在 App 层
        
-   **小白理解**：如果用户自己能打开 CAW 钱包，那关键风控必须在钱包里锁死；如果用户只在 App 里操作，App 层的规则也能兜底。
...
### 1\. 本次 Agent 的定位：不是聊天机器人，而是「自动化业务助理」

这个 Workshop 的核心是：把 [Z.AI](http://Z.AI) 的 Agent 模式，从 “写文案 / 聊天” 升级成一个**能帮你跑完整商业链路的业务伙伴**。

-   目标：不是写漂亮的文案，而是**发现真实可落地的本地商业机会**。
    
-   场景类比：你是一个在泰国芭提雅的外国人，不了解本地市场，Agent 帮你：
    ...
    4.  搭建落地页 + 小范围 Google Ads 测试漏斗
        

### 2\. Agent 的工作逻辑：「人做决策，Agent 干活」

-   你只需要做两件事：**给初始设定**（城市、语言、预算、安全规则） + **在关键节点做选择**（选赛道、选合作方）
    
-   Agent 自动跑完所有执行环节：市场调研、信息收集、工具调用、材料生成
    
-   关键优势：不用自己手动查资料、写文案、搭页面，Agent 帮你把 “从想法到可落地 Demo” 的流程跑通。
    

### 3\. 小白友好的落地步骤

1.  **构建引导式主提示词（Guided Mega Prompt）**
    
    填一次基础信息（城市、语言、预算等），生成主提示词，复制到 [Z.AI](http://Z.AI) Agent 模式里，Agent 会自动引导你走完所有步骤。
    
2.  **核心工作流**
    
    -   市场调研（找需求）→ 筛选细分赛道（选方向）→ 找真实合作方 / 供应商 → 生成落地页 + 广告素材 → 准备小范围测试投放
        
3.  **设计的关键：设置「决策断点」**
    
    Agent 不会一直跑到底，遇到需要人做选择的节点（比如选哪个赛道）会停下来，等你确认后再继续，避免跑偏。
