...
今天我继续推进 Safe Chain Explainer Agent 的黑客松准备，重点不是继续加功能，而是做最终演示排练和答辩准备。

今天我学习到：黑客松提交前最重要的是让项目容易被理解。一个 demo 不只是能跑，还要能在 2 分钟内讲清楚问题、用户、流程、价值和安全边界。

我为项目整理了 demo walkthrough，选择三个最有代表性的 case 来演示：approve unlimited、swap approval、wrong-chain asset confusion。它们分别覆盖授权风险、DeFi swap 前授权风险、L1/L2 错链资产误解。

今天我也练习了项目叙事：这个 Agent 是 read-only，不连接钱包、不签名、不 approve、不 revoke、不转账、不 swap。它只解释公开字段、提示风险、列出用户需要手动确认的信息，并把最终决定权留给用户。

今天还补充了黑客松 Q&A 备答稿，覆盖：项目解决什么问题、为什么是 AI x Web3、为什么 MVP 使用 mock data、为什么暂时不连接钱包、如何减少 AI 幻觉、覆盖了哪些风险场景、安全边界是什么，以及下一步路线。

今天产出：

\- hackathon/[demo-walkthrough.md](http://demo-walkthrough.md)

\- hackathon/[qa.md](http://qa.md)

\- daily/[2026-06-09.md](http://2026-06-09.md)

下一步计划：按照 walkthrough 完整演示一遍，检查 pitch、brief、demo、Q&A 和 readiness checklist 是否能组成一个完整提交包。
