...
今天主要围绕求职定位、产品复盘和岗位投递做推进。

相比前几天的学习笔记，今天更偏执行：重新梳理职业方向，更新 GitHub Profile，搜索岗位，生成投递清单，并完成了 3 个岗位投递。

## **求职 & 产品复盘**

-   用产品视角复盘了 treasury-copilot，明确它的价值和局限。
    
-   判断 treasury-copilot 已经完成黑客松验证，不继续投入迭代。
    
-   完成职业定位升级：
    
    -   从 Web3 开发者
        
    -   调整为 API Integration + AI-assisted Development + Developer Content
...
-   创建并更新 GitHub Profile 仓库：bbainthug/bbainthug。
    
-   推送新版 Profile README，新的职业定位已经上线。
...
补上后可以把今天的记录升级成更完整的求职 proof。

## **今日收获**

今天最大的变化不是多投了几个岗位，而是职业定位更清楚了。

过去容易把自己描述成泛泛的 Web3 developer，但现在更准确的方向是：

> API Integration + AI-assisted Development + Developer Content

这个定位更贴合我已有的项目经历：

-   [LI.FI](http://LI.FI) API / Web3 API 使用经验；
    
-   AI-assisted coding 和 workflow 搭建经验；
    
-   技术 FAQ、评测案例、学习笔记和 developer education 输出；
    
-   能把复杂 AI x Web3 概念整理成清楚的文档和支持材料。
...
-   需要把 GitHub Profile、作品集项目和岗位目标统一成一套面试叙事。
    
-   需要继续补充可展示作品，尤其是 API Support / LLM Evaluation / Developer Content 方向。
...
-   从 29 个岗位清单里继续筛选 5 个高匹配岗位。
    
-   为其中 1-2 个岗位定制简短 cover message。
    
-   把 API Integration + AI-assisted Development + Developer Content 写成一段 30 秒自我介绍。
    

## **Public Post Draft**

Day 16 / AI x Web3 School

今天主要做求职定位和执行。

我重新复盘了 treasury-copilot：它作为黑客松验证已经足够，继续迭代的边际收益不高。更重要的是，我把自己的职业定位从泛泛的 Web3 developer 调整成了更贴近真实经历的方向：

> API Integration + AI-assisted Development + Developer Content

今天也更新了 GitHub Profile 仓库 bbainthug/bbainthug，搜索了 10+ 岗位平台，并生成了岗位投递清单 v2.xlsx：共 29 个岗位，按匹配度和时效做了评分，还包含平台速查和投递记录表。

今天完成了 3 个岗位投递。具体公司和岗位名还需要补进记录。

今天最大的收获是：求职不是把自己包装成“什么都会”，而是找到一个能被项目经历支撑的清晰定位。对我来说，目前最真实、最能讲清楚的方向就是 API 集成、AI 辅助开发和开发者内容 / 技术支持。
