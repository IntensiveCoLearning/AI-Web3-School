...
6/12 之后又断了几天(6/13 到 15),手头别的事压着,连续打卡是真没扛住。今天回来,接 6/12 留的那个最硬的问题:绑定层那张"符号→地址"的可信引用表,到底谁说了算。

第一反应是给编译器内置一张 canonical token list,USDC 就钉到某个地址。写两行就觉得不对:这只是把信任搬了个家——本来要解决"别信环境递来的符号",结果变成"用户得信我这张表"。我自己成了新的中心化信任锚,跟整个项目想去掉中介的方向拧着来。

那靠链上注册表(ENS、各种 token registry)当权威源呢?也不干净:注册表本身可能过期、可能被抢注同名,把"谁是真 USDC"外包给一个我并不能完全信的链上状态,等于把 6/12 那个假 USDC 的风险换了个地方摆,没消掉。

绕一圈才想明白,这问题根上就不该由"某个权威表"来定,该由用户在授权那一刻一次性钉死。DSL 里写 USDC 是为了可读,但真正被 encode、被用户签字认可的,必须是解析后的具体绑定:USDC → 0xA0b8…(Sepolia)、swap → 那个四字节 selector。所以授权流程是:写带符号的 DSL → 编译器解析、把具体绑定摊开给用户看 → 用户确认这份"具体化的 policy" → 才 encode。符号只是糖,被批准的永远是钉死了地址的那份。

这一下那张引用表就不再是信任锚了——它只是个"建议器",提议一组绑定,真正拍板的是用户那一下确认。表错了、或真混进来一个假 USDC,用户在确认界面会看到一个对不上的地址,当场拦下。这跟 5/19 最早定的"写操作必须人工确认"是同一条原则,只是这次确认的不是某一笔交易,而是"符号到底指向链上哪个东西"这层绑定。人审可读的 DSL,签字认的是具体化之后的版本。

今天的产物:把绑定层从"内置权威表"改成"解析-展示-确认"三步,confirm 之前 encode 一律不放行;顺手把这套确认界面的雏形塞进了 6/12 那条 demo 路径。现在 demo 能多讲一段:同一句 DSL,USDC 解析成正确地址时正常授权,故意换成假地址时这步就红。也算半答了 6/12 的"怎么让人五分钟看懂"——最有说服力的不是链上那笔 revert,是用户在授权前就看清自己到底授了什么。下一步把这条带确认的完整流程录成 Week 4 要用的 demo。
<!-- DAILY_CHECKIN_2026-06-16_END -->

# 2026-06-12
<!-- DAILY_CHECKIN_2026-06-12_START -->

昨天说回去建前端,今天把 5/26 那个 permit/session 语法的 parser 写了。parser 本身没什么戏——语法我故意留得很小(5/26 就说要小到 LLM 不会生成歧义),手写个递归下降或套个 PEG 就出 AST 了。真正的活在 parser 之后、encode 之前那一段,在今天之前我一直没正眼看它。

问题是 DSL 里全是人话单位,链上全是机器值,中间差着一层翻译。value <= 50 USDC 的 50,到链上是 50000000(USDC 六位小数);swap 不是方法名,是个四字节 selector;on 后面那个地址得 checksum;expires 2026-07-01 得变成一个 uint48 的 unix 时间戳。AST 还停在人话层,encode 要的是机器值——中间必须插一个绑定/解析的 pass,把符号翻成地址、方法名翻成 selector、日期翻成时间戳。

这一层正是 5/29 说的"语义校验"真正落地的地方。要把 swap 翻成 selector,就得先有目标合约的 ABI,顺带就能查这个 selector 到底在不在这个合约里——不在就是写错了,当场拦;要把 50 USDC 翻成整数,就得知道它六位小数,顺带能挡住"别和 wei 混"这种单位错。当时只是嘴上说"这层最重要",今天写出来才掂出它的分量:parser 是机械的,这层才是 DSL 有没有用的分水岭。

还冒出一个没料到的安全面。USDC 是个符号,它对应哪个合约地址,得查一张表。要是解析时按符号去某个列表里模糊搜,搜到一个同名的假 USDC,那后面 parser 再对、Module 再严都白搭——policy 从源头就绑错了对象。所以这层解析不能"按符号搜",得用钉死的可信引用:固定的 canonical token list、验证过的 ABI。这跟链上验证期"别信任何外部可变状态"是同一个毛病在前端的翻版——别信环境递给你的符号,只信钉死的源。

把这层补上,整条线第一次端到端通了。写一句 permit swap on Uniswap where value <= 10 USDC daily <= 30 USDC,经 parser → 绑定解析 → lowering 分层 → encode 成 config,在 Sepolia 上装到一个 4337 账户的 session key 上;再让 bot 发一笔 8 USDC 的 swap——过了;发一笔 40 USDC 的——被 Module 在链上挡了。从 5/25 立的那个"最小实验"到今天,这条"人话 → 链上强制"的链路头一次完整跑了一遍真交易。

今天的产物:DSL parser + 绑定解析层(含 selector / 单位 / 有效期的语义校验)+ 在 Sepolia 上端到端跑通的一条 demo 路径。也清出了下一批活:绑定层那张可信引用表怎么管(token list 和 ABI 从哪来、谁背书),以及把"40 USDC 被当场挡下"做成可复现的 demo 脚本——Week 4 hackathon 要拿出去讲,光跑通不够,得让人五分钟看懂"一句话怎么在链上拦住一笔越权交易"。
...
昨天把 encode + 通用 Module 的解释骨架搭好了,今天该兑现那句"用同一套 stateful 测试验证解释器版本和硬编码版本行为一致"。把 6/8 那套等价测试的被测对象从硬编码 Module 换成"通用 Module + 配置",重跑——绿了。解释器对任意配置,逐笔 verdict 和 (cumulative, lastReset) 状态都和 DSL 参考模型一致。链上那半在功能上算定型了。

但顺手测了下 gas,问题就冒出来了。解释器每次 validateUserOp 都要把那段 ABI 编码的配置 decode 出来、再循环逐条判,这笔开销随约束条数线性涨。约束一多,验证期的 gas 逼近 verificationGasLimit——功能对了,但贵且不稳定。

想清楚后发现这是个时机错配:我把"解析配置"放在了每笔交易都要跑的验证期,而它其实只需要在 session 安装时跑一次。配置一旦定下,后面每笔交易读的应该是已经解析好、定长摆放的状态,而不是每次重新 decode 一遍变长的 ABI bytes。

所以拆成两段:安装期(给账户装这把 session key 时,走的是执行期那条路,gas 贵点无所谓)把配置 decode 出来、按定长布局打包写进账户的关联 storage;验证期只读这块定长 storage,不再 decode、不再遍历变长结构。这正好又落在 5/31 那条上——验证期只能碰账户关联 storage,而这块预解析的配置就存在那儿,合规且便宜。等于"贵的活付一次,每笔交易只付便宜的读"。

退一步看,整个 Week 3 到今天,我一直在链上这半边打转:Module、validateUserOp、执行期、解释器、gas。等价测试绿了,这半边的形状就定了。但 SessionGuard 还有另一半一直停在草图上——真正把人写的 DSL 文本 lex/parse 成 AST 的前端。我手上一直是"假设已经有 AST"在往下推 lowering 和 encode,前端那几步还没真写。

今天的产物:解释器版等价测试通过 + 安装期预解析、验证期读定长的 gas 优化方案(还没实现,先记进 spec)。明天换方向,回去建前端:把 5/26 那个 permit/session 语法真正写出一个 parser 来,让"一句人话 DSL → AST → encode → 链上 config → 一笔受限交易"这条线第一次端到端跑通。这条跑通了,Week 4 hackathon 才有能 demo 的东西。
...
隔了一天没打(6/9),今天补上,继续昨天说的 codegen——把 lowering 分好类的原子约束生成成链上能跑的东西。结果"生成成什么"这个问题本身,先把我拦下了。

我一直默认 codegen 就是生成 Solidity:一条 policy 编译出一个量身定做的 SessionKeyModule 合约,把约束硬编码进去,再编译、部署。写到一半反应过来这条路根本不通——每来一个新 session 就要部署一个新合约,慢、贵、UX 灾难;更要命的是安全:用户怎么审一段"现编现译出来的字节码"?这恰恰把 DSL 当初要消掉的问题(5/29、6/6 一直在说的"让人审可读的东西")原样请了回来。

正确的目标不是代码,是数据。codegen 应该输出一段编码后的约束配置,喂给一个事先部署好、只审一次的通用 SessionKeyModule;Module 在运行时解释这份配置来强制约束。用户审的是人话 DSL,链上信的是那一个审计过的 Module,中间靠 6/7、6/8 那套等价测试担保"Module 解释配置 ≡ DSL 语义"。一次审计,所有 policy 复用。

这一下把整条编译链的性质看清楚了:Module 不是被生成的,是个解释器;编译器的后端不产代码,产数据。这也正好解释了为什么那条等价 property test 这么关键——它不是验证某一次生成的合约,而是一劳永逸地证"这个审计过的解释器,对任意配置都按 DSL 语义执行"。同时接上 5/26 说的可移植性:换 provider 不过是同一棵 AST 编码成另一个通用 Module 认的格式,源 DSL 不动。

但解释器不是白来的。通用 Module 在 validateUserOp 里要循环遍历约束配置去逐条判,这是 gas,而 validateUserOp 的 gas 又卡在 verificationGasLimit 里。这反过来又给 6/3 那条"静态约束进验证期、有状态的进执行期"补了一个理由:验证期那组越小,解释器在验证里的循环越短、越省 gas。当时是为了绕 opcode 限制才拆,现在发现拆完顺带把验证成本也压下去了。

今天的产物:把"codegen"正式改名成"encode",后端从生成 .sol 改成生成约束配置的 ABI 编码;通用 Module 加了读配置、按 type 分发到对应检查的解释骨架。还没把 6/8 那套 stateful 测试重新指到"通用 Module + 配置"这个组合上跑——明天就干这个,等于用同一套等价测试,验证解释器版本和之前硬编码版本行为一致。如果绿,SessionGuard 的链上那半就算定型了。
