...
[https://x.com/i/broadcasts/1RKjpzPzVzZJw](https://x.com/i/broadcasts/1RKjpzPzVzZJw)

# Web3 架构的第一原则是不可逆

看了 TC 的回放「AI 时代 Web3 开发者需要的基础知识和架构能力」。里面有一句话直接把问题核心捅穿了："如果基础知识和架构能力不足，就无法判断 AI 给上来的是原石还是掺杂。"

他讲了三个粉丝问题，我觉得第三个最值得想：**"安全"可以后续再考虑吗？**

答案是不行。Web3 直接跟资金交互，安全不是一个后面可以打的补丁，而是架构设计的前置约束。"先跑起来再说"在后端可以，在链上不行——出了事没有回滚按钮。"审计可以补"也是错觉——审计能发现问题，但补不了已经被转走的钱。

这个判断跟我这两天做 Week 1 任务的体感完全吻合。我设计 Restricted Web3 Helper 的时候核心也是这个思路：不是先让 AI 能做所有事再加限制，而是从一开始就把"不能做什么"当作架构骨架——读链放行，写链拒绝，签名必须人工。先定边界，再填能力。

另一个收获是关于人和 AI 的协作边界。TC 把它分了三层：人负责设计方案和评估合理性；AI 协助设计和优化、自跑验证；多 Agent 框架做自动化构建部署。关键是第一层不能省——如果人自己看不懂 storage layout 或者不理解 reentrancy，AI 生成的代码编译过了也不代表安全。

今天还把 Week 1 全部提交做了第二轮精简，砍掉了所有过程描述和指标汇报，只留链接 + 一句话索引 + 主观判断。
