...
6 月 14 日 · AgentCFO 终章

▎ 写给 14 天前的 threetwoa。如果有下次，我会回来看这一页。

一句话

我们在 Cobo Agentic Wallet 的边界内，搭了一个由 AI 出计划、由 Risk Engine 拍板、由人类最后签字的 DAO 财务官。这不是最炫的 Web3 项目，但它是把"agent

能不能管钱"这件事认认真真拆成可讲清楚、可跑通、能在评委面前崩盘后还能讲清楚的闭环。

Slogan 我们写在 README 第一屏：Give every DAO an AI CFO with a controlled wallet。前半句是 user value，后半句是 boundary。这不是营销话术，这是产品定位本身。
...
│ Backend P0 │ ✅ │ 4+1 端点，mock 默认 + opt-in real CAW；Render 上跑着 │

├──────────────────────────────────┼──────┼─────────────────────────────────────────────────────────────────────────────────────────┤

│ P2 (demo-safe) │ ✅ │ external refs / Request Finance / Sablier preview / multi-agent treasury mock │

├──────────────────────────────────┼──────┼─────────────────────────────────────────────────────────────────────────────────────────┤

│ CAW testnet evidence │ ⚠️ │ 2 笔（0x85a5…0ed4d98 demo payment + 0x6bd7…bae8a internal transfer） │

├──────────────────────────────────┼──────┼─────────────────────────────────────────────────────────────────────────────────────────┤

│ Frontend Landing │ ✅ │ Velorix Hero + Pipeline Editorial + 8 张 showcase │

├──────────────────────────────────┼──────┼─────────────────────────────────────────────────────────────────────────────────────────┤

│ Frontend Console │ ✅ │ 5 模块（Agent Hub · Treasury · Wallets · Analytics · Policy）Phase 7 + Phase 8 视觉升级 │

├──────────────────────────────────┼──────┼─────────────────────────────────────────────────────────────────────────────────────────┤

│ PPT / 海报 / 头图 / 团队 3D 形象 │ ✅ │ ppt-master 14 页 + 物料 PDF + Banner + 6 套 3D 形象 │

├──────────────────────────────────┼──────┼─────────────────────────────────────────────────────────────────────────────────────────┤

│ Demo 视频 │ ✅ │ 1m50s 短版已就位 Landing #guardrails 手动播放 │

└──────────────────────────────────┴──────┴─────────────────────────────────────────────────────────────────────────────────────────┘

mock 标得清清楚楚。blocked 项不进入 CAW。Audit Report 是执行时快照。三件事我们写在 README 第一屏——这不是怯，是态度。
...
Payment Plan 用 AI 生成（含付款原因、汇总），但 Risk Engine 是确定性的——它不读 prompt，只看白名单、预算、单笔限额、是否重复、token 是否允许。Bob 的 15 USDC

因为钱包不在白名单直接被 block。这条纪律让我们有一个清晰的故事线可以讲，而不是 "AI 替你拍板了"。

2\. Mock by default，real by opt-in。

CAW 真链上付款要 env、要 policy、要 allowlist，缺一就 fail closed。Render 默认 mock mode——这一刀切下去之后，所有 demo 流程可以稳定地跑；要不要切 real，是 purple sun

与评委的对话，不是 threetwoa 与 Render 日志的对话。

3\. Human Approval 是真东西。

Demo 的故事不是"看，AI 一句话发了 50 USDC"，而是"看，我们挡住了一次错误付款"。这让项目有了立场。 一个 agent 钱柜如果只会发钱，是工具；如果会挡钱，才是同事。

4\. Audit Report 是快照。

执行时定格，之后任何 CAW status 刷新都不改写它。审计者和运行者看的是同一份事实——这是合规的最低要求，也是别人愿意把 DAO treasury 交出来的最低要求。

5\. 视觉不是装饰。

Landing 的 Pipeline Editorial 是给评委 5 秒钟看懂的"双轨瀑布流"——03 贴 01、04 贴 02。Console 五模块的 hero header、Treasury GSAP 水平滚动、Policy guardrail 拓扑、Navbar

路由过渡——每个都有功能上的原因，不只是好看。Landing 锁定的不是像素，是叙事。

\---

没讲完的故事（这些是真实的欠账）

\- 第三条 testnet tx 没凑齐。 README 里把第二条 internal transfer 单独标注："不替代 demo payment 证据"。赛方若按"3

笔独立商业付款"口径，我们会少一笔。这是有尊严的诚实题——比硬凑一个更有尊严，但下次必须 Day 8 就做完，不要等。

\- real mode 联调没线上闭环。 Treasury 已接 API、Vercel 仍 mock、Render URL 待填。最后两天用来打磨视觉和答辩稿，这是取舍，也是欠账。Next time：联调窗口在 build period

中段就打开，别压到 48 小时。

\- Demo 视频在最后一晚还在改路径。 1m50s 短版 → GitHub raw URL → LFS 排除 → Vercel CLI 排除——这一连串 commit

看得我心疼。视频不是工程问题，但把视频当工程问题处理，是这次学到的最有用的一条。

\- Landing 在最后一晚还有大改。 这是错的。如果回到 Day 9，我把 Landing 锁掉 72 小时，那 72 小时给 Console 细节和答辩稿。

\---

答辩现场

今天 5 分钟上台，我讲了：为什么是 DAO、为什么是 Cobo、为什么是这个 6 人流程、为什么 Bob 被挡住。Q&A 几乎全问到了 Risk Engine vs LLM 的边界、mock vs real 的策略、第三条 tx

是不是真——问到的几乎都是我们花时间最认真的那几个点。

这说明两件事：

\- 准备过的位置就是被问的位置

\- 草率带过的位置不会有人问，但也没人会记住

答辩最大的教训不是讲得好不好，是少讲了多少。 我们能讲的内容远多于 5 分钟。下次要在 Day 9 就想清楚"5 分钟只能讲 3 件事"，第 4 件往后留。

\---

真正带走的东西

\- "会挡错的 agent" 比 "会发钱的 agent" 难十倍，也重要十倍。 AgentCFO 不是"AI 替 DAO 发钱"，是"AI 是 DAO 财务室里那个会做方案、但不会按回车的同事"。这是这个产品真正的命题。

\- 风险边界写在 README 第一屏 是一种态度：我们不怕你看穿我们怎么挡错。

\- 团队没有单点。 PM 拆 scope、前端不画后端图、后端不接前端线、purple sun 单独养 CAW、呱呱把每一帧做成可复用资产、ZanyK 把"总控"做成缺席两次也能跑的会议桌。6 个人 14

天，这不是协调的胜利，是边界的胜利。

\- Claude Code 这次真的是伙伴。 多 Agent 并行 + scope commit + phase handoff，没有这套工作流做不完。

\- 17 个 commit 名字里有同一个字：fix——比 feat 多。这不丢人，这是事实。
...
1\. README 在 Day 1 就写 v0。 提交前 72 小时写是错的。文档是基础设施，不是装饰。

2\. 第三条 tx 在 Day 8 做完。 不要等。

3\. 联调窗口在 Day 7 打开。 不要等。

4\. 视觉锁定在 Day 10。 不要等。

5\. 答辩稿在 Day 9 开写。 不要等。

6\. Landing 永不大改 72 小时内。 锁就是锁。

7\. Mock 与 Real 不是切换是两种产品。 答辩时要分两个故事讲。

我未必能做到。但我把这页打开。

\---

写在 2026/06/14，Demo Day 之后。

不管结果如何，这 14 天我们交出了一个会停下来、会挡住、会让人类签字的 agent。
...
今天是 6 月 13 日，提交截止日。我把早上到中午做的事整理成一份梦想营打卡笔记，按你的语气写：
...
**角色：** 前端 threetwoa  
**时间：** 约 9:40 — 12:45  
**仓库：** [https://github.com/San-Y108/agent-cfo](https://github.com/San-Y108/agent-cfo)  
**线上 Demo：** [https://agentcfo-frontend.vercel.app](https://agentcfo-frontend.vercel.app/)

* * *

### **今天干了什么（一句话版）**

把 AgentCFO 从「能跑」推到「能交」——README 首页打磨、Console 五页截图补全、Demo 视频接上 Landing，前后端 Agent 聊天打通，最后 push 到 GitHub。

* * *

### **详细记录**

**1\. README 首页合并与文档对齐（~9:42）**

-   按 `docs/plans/README-merge-plan.md` 把根目录 README 从纯技术手册改成赛方可读的首页
    
-   加了赛事信息、Cobo 赛道匹配、团队表、Landing/Console Showcase 结构
    
-   合并前完整版备份到 `docs/backup/`，同步 `AGENTS.md`、`CLAUDE.md`、`docs/pm/SUBMISSION_CHECKLIST.md`
    
-   这一步是交付底座——评委第一眼打开 GitHub 就能看到完整故事
    

**2\. Console Command Center 大升级（~10:09 — 10:35）**

今天前端主战场在这里，一口气把 Console 从骨架推到可路演状态：

| 模块 | 做了什么 |
| --- | --- |
| 整体设计 | 设计 token、Console 浅色主题、Command Deck HUD 组件 |
| 路由架构 | 五个页面拆成独立 stage 模块：Agent / Treasury / Wallets / Analytics / Policy |
| Agent Hub | 刷新聊天工作台，加 agent 吉祥物，中英文文案 |
| Policy | Guardrail 拓扑改成横向三列布局（Tier 1/2/3 → CAW Core） |
| Analytics | 饼图侧栏 + 支出趋势图，修复深浅色主题下切片颜色 |
| Wallets | Vault 拓扑图适配深浅色主题 |
| HUD | 浅色模式下 accent 对比度修复 |

**3\. Agent Hub 聊天打通 MiniMax（~10:35 — 10:36）**

和后端九九八乂对齐，把 Agent 页从「假聊天」变成真代理：

-   后端新增 `POST /api/agent/chat`（MiniMax 代理，Key 只在后端）
    
-   前端 `lib/api/agent.ts` adapter 镜像契约
    
-   README API 表和文档同步更新
    

**4\. Demo 视频接入 + Vercel 部署踩坑（~10:51 — 12:28）**

呱呱那边 demo 视频做好了，我负责接到产品里：

-   写 `sync-demo-video` 脚本，Landing `#guardrails` 区可播放
    
-   **踩坑：** 视频太大，Vercel CLI 部署直接挂
    
-   **解法：** 视频放 `frontend/public/video/`，构建时 sync；GitHub 用 Git LFS 保留源文件 `assets/video/agentcfo-demo.mp4`
    
-   最终 Landing 可播、Vercel 能部署、GitHub 有源文件，三条线都通
    

**5\. README Showcase 截图补全（~11:02 — 12:42）**

这是今天中午和 Cursor 一起收尾的重点。对照 Landing 那套 3×3 表格，把 Console 也补齐：

**Landing Page（8 张，之前已有）：**  
Hero / Pipeline / Platform / Guardrails / Timelines / Built by Teams / FAQ / Footer

**Command Center（5 张，今天新截）：**

| 页面 | 路由 | 截图文件 |
| --- | --- | --- |
| Agent Hub | /console | console-agent-hub.png |
| Treasury | /console/treasury | console-treasury.png |
| CAW Wallets | /console/wallets | console-wallets.png |
| Analytics | /console/analytics | console-analytics.png |
| Policy | /console/policy | console-policy.png |

用 Playwright 对本地 dev server 自动截图，不用手截。

**修了几个显示 bug：**

-   **Banner** 之前是裁切版（1200×400），上下卡片被截掉 → 换回 `inbox/banner.png` 完整版（1672×941）
    
-   **Agent Hub 截图** 之前误放了吉祥物宇航员图 → 换成真实 Console 界面
    
-   **团队头像** 原始图最大 1075px，GitHub 上撑爆表格 → 改成 `width="80"` 限制
    
-   **标题** 从一行挤满的 H1 拆成三层：`AgentCFO — DAO AI Treasury` / 英文 tagline / 中文描述
    
-   **赛事状态** 从「待提交」改成「✅ 已提交 · Demo Day 6/14」
    

最后 commit `4719546` push 到 main。

* * *

### **今天交付了什么**

-   README 首页可给评委直接看（结构 + 截图 + 赛事信息）
    
-   Console 五页全部有 Showcase 截图
    
-   Demo 视频在 Landing 可播、Vercel 可部署
    
-   Agent Hub 真聊天（MiniMax 代理）
    
-   代码已 push GitHub main
...
截止日最怕的不是功能不够，是**评委打开 README 第一眼觉得「没做完」**。Landing 有 8 张图、Console 只有 1 张吉祥物，观感差很多。截图、Banner、头像尺寸这些「最后一公里」看着小事，实际上决定 GitHub 首页是否像正式交付。

另外 Vercel 部署大文件要分开处理——产品里用 `frontend/public/`，仓库里用 LFS，别硬塞一个路径。
...
\- Pipeline Editorial 完整重构 — 5 stage 纵向流 + 侧边 timeline 导航 + 字符解码动效

\- Hero 标题接入 DecodeHeadline（字符解码 + 鼠标排斥），"DAO"/"executable" 关键字发光艺术体

\- Navbar 滚动行为优化 — pastHero 时丝滑收缩为居中 floating pill（Framer Motion transform）

\- Footer glitch-art 重设计、Timeline 胶片帧亮度修复、Web3 星座网络节点可见性修复

\- 系统级亮度修复全站 pass、FAQ 间隙移除

Console 升级（Phase 1-3）

\- Phase 1: ambient orbs + GradientOrb 扩展 + GSAP SlamText/ScrambleValue

\- Phase 2: Treasury Header/KPI/Records/Action Panel 视觉升级 + txHash GradientText + RiskGate Scramble

\- Phase 3: Wallets/Analytics Header + card stagger + hover glow

\- Phase 0 Taste-Skill baseline fixes 重新应用

工程修复

\- DecodeHeadline 从 per-char 改为 per-word 渲染，解决单词被拆断问题

\- ShootingStars 内存泄漏修复（clearTimeout cleanup）

\- Video hydration warning 修复（suppressHydrationWarning）

文档 & 流程

\- Phase 0-3 execution checklist 持续更新（34/58，59%）

\- Session handoff Phase 1-2-3 文档

\- PM: backend sick 任务重新分配、CAW demo 话术任务新增

Git & 部署

\- feat/console-aceternity-upgrade → feat/frontend-dev → main merge ✅

\- Vercel 生产部署：[https://agentcfo-frontend.vercel.app](https://agentcfo-frontend.vercel.app)
...
\- 问题：Workflow section 被 GSAP pin 固定后，IntersectionObserver 检测不到

\- 解决：加了一个 #workflow-marker 非 pinned 标记元素，导航和 observer 都观察它

2\. Hero 标题消失

\- 问题：粒子标题组件把原来的文字遮住了，只剩空白

\- 解决：文字始终显示在底层，canvas 粒子叠加在上方

3\. Console 表格报错

\- 问题：<tr> 里直接塞了 <div>，React hydration 报错

\- 解决：把左侧色条改成 <td> 的左边框样式

4\. Timeline 文字重叠

\- 问题：所有 Phase 文字和胶片帧同时显示，叠在一起像鬼影

\- 解决：给非活动元素加初始 opacity: 0，ScrollTrigger 刷新时强制同步状态

\---

✨ 新功能

5\. Footer AGENTCFO 大字特效

\- 鼠标悬停时 SVG feTurbulence + feDisplacementMap 产生水波纹扭曲

\- RGB 分离（青色/洋红两层）造成数字撕裂感

6\. 背景粒子纹理

\- Canvas 2D 点云背景，暗色像素化粒子缓慢飘动

\- 鼠标靠近时有涟漪排斥效果

7\. Hero 标题粒子效果

\- 标题文字被拆成粒子，鼠标划过会推开粒子形成"数字撕裂"

\- 粒子颜色用 HSL 彩虹渐变

8\. Timeline 胶片重做

\- 从"小玩具"升级为 cinematic 大胶片

\- 36vw × 64vh 大尺寸，带齿孔、暗角、film grain

\- 去掉卡通图标，改用阶段数字（01/02/03/04）

\- 颜色从荧光绿改为沉稳色调

9\. Console 风险拦截动画

\- 有 blocked payment 时，KPI Cards 上方弹出红色抖动动画提示
...
\- Hero 粒子方案调研（Canvas + SVG + CSS glitch 三层叠加）

\- TRAE 效果融入方案（cinematic dark interface 方向）
...
# 残酷共学营打卡｜AgentCFO 前端从“能想清楚”推进到“能跑、能演示、能提交”

今天主要推进的是 AgentCFO / DAO AI 财务官的前端项目。

之前这个项目更多还停留在“结构、文档、mock 数据、handoff 资产已经准备好”的阶段，但还不是一个真正可运行、可打开、可演示的前端应用。今天把它往前推了一大步：从 workspace 变成了一个可以跑起来的 Next.js 前端 demo。

## 今天完成了什么

### 1\. 手动初始化了 Next.js 项目

没有直接用 `create-next-app` 粗暴覆盖，而是走了 Plan B：手动补齐最小可运行工程。
...
这个阶段的意义是：项目不再只是文件夹和文档，而是一个真实可运行的前端应用。

## 2\. 做出了 AgentCFO Demo Console v0

今天最核心的产出是 `/demo` 页面。

它现在已经不是一个占位页，而是一个完整的 mock-first demo console，包含 6 个核心区块：

1.  Agent Workflow Timeline
    
2.  Payment Plan Board
    
3.  Risk Gate
    
4.  Human Approval
    
5.  Simulated CAW Execution
    
6.  Settlement Receipt / Audit Report
    

整个 demo 讲的是一个非常具体的故事：

一个 DAO 需要支付 3 位贡献者 + 1 个工具订阅，总计划金额 50 USDC。AgentCFO 生成付款计划，执行风险检查，发现 Bob 的钱包地址不在白名单里，于是阻断这笔付款；其余 3 项付款进入人工审批，并模拟通过 Cobo Agentic Wallet 执行，最后输出 settlement receipt。

这个 demo 目前没有接真实钱包，也没有真实链上交易，但 UI 流程已经完整闭环。

## 3\. 整理了 mock-first 数据流

今天也做了一个重要的小架构处理：没有让页面直接散落读取各个 mock 文件，而是用 `lib/demo/demo-data.ts` 作为统一数据出口。
...
这样后面如果要从 mock 切到真实 API，不需要重写整个页面，只需要替换数据来源。

这个设计对 hackathon 很重要，因为短期要先保证 demo 稳定，后续再接真实 Cobo Agentic Wallet 或后端 API。

## 4\. 明确了 mock 和 real 的边界
...
我觉得这个边界很重要。Hackathon 可以做 demo，但不能把 mock 说成 real。现在 README、demo runbook 和 submission 文档里都明确写了 simulated / mock-only，避免后面路演时表达过头。

## 5\. 完成了 hackathon 提交材料包

今天还整理了几份提交相关文档：

-   `submission-snapshot.md`
    
-   `demo-runbook.md`
    
-   `submission-form-pack.md`
...
这个对我来说不只是写文档，而是把项目叙事强行收敛了一遍。

现在 AgentCFO 的核心叙事比较清楚：

> 从贡献记录到付款计划，从风险检查到人工审批，从模拟钱包执行到审计报告。

## 6\. 做了一轮 UI 资产扫描

今天最后还让 Claude Code 扫描了几个本地 UI 资产库，为下一轮视觉升级做准备：
...
当前 AgentCFO 已经工程闭环，但视觉还比较基础。下一轮需要从“能演示”推进到“像一个真正的 AI Treasury Command Center”。

报告里初步推荐的方向是：

-   暗色主调
    
-   金融 / treasury control 感
    
-   更强 dashboard 结构
    
-   KPI、Widget、DataGrid、AppLayout
...
这也基本符合我后面对视觉方向的判断：AgentCFO 不能做成普通 SaaS 页面，它应该更像一个 Web3 财务控制台。

## 今天的几个关键判断

### 判断 1：先跑起来，比先追求完美视觉更重要

今天没有一开始就上 HeroUI Pro、Aceternity、GSAP，也没有直接把页面做成复杂官网。

先把 Next.js 跑起来，先把 demo 闭环做出来，这个顺序是对的。

因为 hackathon 最怕的是：视觉做了很多，但演示路径不稳定。

### 判断 2：mock 可以接受，但边界必须明确

当前还没有真实 Cobo Agentic Wallet 集成，这是项目短板。

但只要明确说这是 frontend mock demo，并且把 permission boundary、human approval、auditability 的 UI 和数据流做出来，它仍然可以成为一个有效的 hackathon MVP。

### 判断 3：今天真正完成的是“提交准备度”

今天的价值不只是写了几个组件，而是把项目推进到了可以被提交、可以被讲、可以被录屏、可以被别人理解的状态。
...
这对 hackathon 来说比单纯多做一个功能更关键。

## 当前遗留问题

还没做：

-   真实 Cobo Agentic Wallet testnet execution
...
1.  继续增强 demo 的金融产品可信感
    
2.  准备 demo 视频
    
3.  明确是否能接一小段真实 Cobo testnet execution
    
4.  把 Landing Page 从“入口页”升级成真正的产品官网
    

## 今日总结

今天算是把 AgentCFO 前端从“项目想法和目录结构”推进到了“可运行 demo + 可提交材料”的阶段。

这不是最终形态，但已经跨过了一个关键门槛：现在它不是只存在于文档里的项目，而是一个可以打开、可以展示、可以讲清楚边界和下一步的 hackathon demo。

明天重点应该不是继续随机加功能，而是围绕提交和路演倒推：

-   demo 视频怎么录
    
-   首页怎么更像产品
    
-   Demo Console 怎么更像金融控制台
...
2026-06-08 今日打卡｜AgentCFO 前端推进记录

今日主线

今天主要围绕 AgentCFO 前端从“能跑”推进到“能展示、能协作、能继续优化” 展开。
...
团队仓库正式前端 + Vercel 线上演示 + Landing / Demo 初步成型
...
旧个人实验仓库 hackathon-frontend 已不再作为正式开发仓库。

当前正式工作区切换到：

D:\\OneDrive\\Desktop\\threetwoa\\my-competition\\agent-cfo\\frontend

前端代码已迁入团队仓库 San-Y108/agent-cfo。
...
PR #1 feat/frontend-bootstrap → main

\---

2）Vercel 部署完成

今天完成了团队仓库前端的正式 Vercel 部署。
...
关闭了 Vercel Authentication。

修复了 deployment protection 导致的 401。

修复了非 production / alias 映射导致的 404。
...
前端不能自己发明 endpoint / field / response wrapper

已明确规范：

后端 app/models.py + app/routers/payments.py + tests/test\_mvp\_flow.py
...
workflow 层 real HTTP chain 曾验证可跑
...
/demo 页面 real mode UI 接入

\---

4）前端规范文档落地

已让 Claude Code 在 frontend/ 下生成：
...
CLAUDE.md：前端总纲、防 API 幻觉规范、工作边界。

checklist.md：前端任务清单。

backend-integration.md：前后端联调记录。

这一步解决了后续 Claude Code / Codex 接手时上下文丢失的问题。

\---

5）Landing Page 完成初版

产品官网首页 / 已完成 Velorix IIC 风格改造。
...
6）Demo Console 完成大改版

/demo 已从原本不好看的静态卡片，改造成更像业务工作台的结构。
...
Intro → Payment Plan → Risk Check → Approval → Execution → Audit
...
Cobo Agentic Wallet execution 展示

tx hash / request id copy

sparkles / beam / hover glow 等视觉增强

\---

7）Aceternity 资产分析与部分应用
...
亮暗模式影响 Hero 下方 sections 和 /demo。

中英文切换需要覆盖 Landing 与 Demo。
...
继续把前端 commit 累积到 PR #1

优化告一段落后统一 review + merge
...
\> 前端先行时，不能让 Claude 自己脑补 API。

后续规则：

所有 API 调用必须先读后端 contract。
...
Aceternity / MotionSites / HeroUI Pro 这些资产要作为素材库，而不是整包塞进项目。
...
3）Landing 和 Demo 是两个不同产品形态

今天明确：

/ = 产品官网首页，用于第一眼高级感和项目解释

/demo = 业务工作台，用于展示 AgentCFO 流水线
...
P1：等待后端 Render URL + CORS

后端给出 Render URL 后，前端需要：

NEXT\_PUBLIC\_DEMO\_MODE=real

NEXT\_PUBLIC\_API\_BASE\_URL=<Render URL>

然后完成：

/demo real mode UI 接入
...
README / PPT / demo video 素材整理

\---

6\. 今日打卡一句话

今天完成了 AgentCFO 前端从迁移、部署、API 规范、文档沉淀，到 Landing / Demo UI 初步成型的一整轮推进；当前前端已经能在线上展示 mock demo，真实联调等待后端 Render URL 与 CORS，下一步重点是收尾主题/语言切换并准备 real mode 接入。
...
# 2026年6月7日 学习打卡｜AgentCFO 前端项目工作流梳理

今天主要推进的是 AgentCFO / DAO AI 财务官这个 hackathon 项目的前端工程地基和 AI Coding 工作流。

今天不是单纯“写了多少代码”，而是把整个开发流程重新梳理了一遍：GPT 负责做决策层和综合层，Claude Code 负责本地执行开发，Codex / Claude Code 可以作为 review 层，我自己作为项目 owner 做最后判断。这个分工对我来说挺重要的，因为这样不是盲目让 AI 一路自动写，而是每一步都先讨论目标、边界、风险和验收标准，再把精心设计好的 prompt 交给 Claude Code 执行。

项目上今天走的是 Plan B：不使用 `create-next-app`，而是在已有仓库结构上手动补 Next.js / TypeScript / Tailwind v4 的基础配置。这样可以避免覆盖原本已经整理好的 `app/`、`lib/`、`handoff/`、`ai/`、`docs/` 等目录。Claude Code 已经完成了本地初始化，补齐了 `package.json`、`tsconfig.json`、`next.config.ts`、`postcss.config.mjs`、`app/layout.tsx`、`app/globals.css` 等基础文件，并且 `pnpm typecheck`、`pnpm build`、`pnpm dev` 都通过了。也就是说，前端项目已经从“文档和结构工作区”变成了一个真正可以运行的 Next.js 项目。

今天另一个比较关键的点是，我把 GitHub 仓库连接器也接进来了。这样后续 GPT 不只是靠我贴 Claude Code 的报告来判断，而是可以直接基于 GitHub 远端仓库去看 commit、文件和 diff。也就是说，之后每次 push 之后，都可以形成一个更完整的闭环：Claude Code 执行 → 我本地 review → push 到 GitHub → GPT 通过连接器审查真实产出 → 必要时再让 Codex/Claude 做反向 review → 最后再设计下一轮 prompt。

今天也梳理了接下来项目的整体阶段：当前处于 Next.js 初始化后的人工 Review 阶段，接下来会先确认本地 diff 是否安全，再 push 到 GitHub，然后进入 `DESIGN.md` / UI 方向沉淀，再做 Mock-first Demo Console，之后再考虑 HeroUI Pro、视觉增强、API 联调、Cobo Agentic Wallet 展示、Vercel 部署和最终路演材料。

我今天最大的收获是：AI Coding 不是把任务丢给模型就结束，而是要设计一套可控的协作协议。Prompt 不是随便写一句“帮我开发”，而是要明确本轮目标、允许修改什么、禁止修改什么、如何验证、输出什么报告，以及我自己应该在 Cursor 里看哪些文件。这个过程让我更像是在搭建一个自己的 AI 开发系统，而不是单纯使用某个 AI 工具。

今天的状态总结：AgentCFO 前端地基已经搭起来了，下一步先做人工 review，把控制权拿回来，然后再进入 UI workflow 和 Mock-first Demo Console 的正式开发。
