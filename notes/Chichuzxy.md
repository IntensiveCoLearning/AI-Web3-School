...
项目: ZKML Pipeline Agent | 赛道: [Z.AI](http://Z.AI) Long-Horizon Task | Demo Day
...
### **8\. write\_file / execute\_code 路径 double-prefix**
...
-   现象: calibrate\_settings 等函数参数从 dict 改为文件路径
    
-   解决: 通读 API 文档，全部改用 keyword args
    

### **10\. create\_evm\_verifier async bug**

-   现象: Windows 下 "no running event loop" (pyo3 tokio 线程问题)
    
-   解决: [asyncio.run](http://asyncio.run)(create\_evm\_verifier(...)) 手动包装
    

### **11\. Foundry via\_ir 编译要求**

-   现象: Verifier.sol Stack too deep
    
-   解决: foundry.toml via\_ir = true
    

### **12\. vm.envUint hex prefix 问题**

-   现象: envUint 读 .env 的 PRIVATE\_KEY 带 0x 前缀导致铸造失败
...
-   现象: id\_ed25519 不是 Chichuzxy 的 GitHub SSH key
...
### **23\. 手动 eth\_abi.encode 格式错误**

-   现象: 自己用 eth\_abi.encode(\['bytes','uint256\[\]'\], \[proof, instances\]) 编码 calldata
    
-   根因: Verifier.sol 内部对 instances 的编码格式与手动编码不完全一致
    
-   解决: 必须用 ezkl.encode\_evm\_calldata() 生成 calldata
...
-   根因: Day 2 生成 proof.json，Day 3 单独跑 create\_evm\_verifier，中间 setup 可能被重跑导致 VK 变化
    
-   解决: 删所有产物，一键从头跑完 setup+prove+create\_evm\_verifier
...
-   project/[README.md](http://README.md) (完整 demo 说明)
    
-   project/artifacts/verifier-address.txt (合约+部署TX)
    
-   project/artifacts/verify-evidence.txt (链上验证证据)
    
-   project/artifacts/[risk-boundary.md](http://risk-boundary.md) (风险边界)
    
-   根 [README.md](http://README.md) Week 4 状态更新
    
-   agent-workflows/run\_[demo.py](http://demo.py) (端到端编排脚本)
    
-   agent-workflows/web\_[ui.py](http://ui.py) + templates/ (Demo Web UI)
...
### **2\. run\_**[**demo.py**](http://demo.py) **修复**
...
-   project/[README.md](http://README.md) 重写
    
-   project/artifacts/verify-evidence.txt (新增)
    
-   project/artifacts/[risk-boundary.md](http://risk-boundary.md) (新增)
...
gen\_settings -> calibrate -> compile -> gen\_srs -> setup -> gen\_witness -> prove -> verify -> create\_evm\_verifier

-   9 步一次性跑完, VK 与 proof 绑定
    
-   encode\_evm\_calldata() 生成 calldata, 不能手动 eth\_abi.encode
...
项目: ZKML Pipeline Agent | 赛道: [Z.AI](http://Z.AI) Long-Horizon Task
...
Day 5 根因定位: 手动 eth\_abi.encode(\['bytes','uint256\[\]'\], \[proof, instances\]) 编码的 instances 格式与 Verifier.sol 内部期望格式不一致。 ezkl 23.0.5 必须用 encode\_evm\_calldata() 生成 calldata。

操作步骤:

1.  删除 pk.key / vk.key / proof.json / Verifier.sol（清空所有产物）
    
2.  修复 ezkl\_[pipeline.py](http://pipeline.py): create\_evm\_verifier 用 [asyncio.run](http://asyncio.run)() 包装
    
3.  一键跑 python ezkl\_[pipeline.py](http://pipeline.py)（setup -> prove -> create\_evm\_verifier）
    
4.  forge build + forge script deploy --broadcast
    
5.  ezkl.encode\_evm\_calldata() 生成正确 calldata
...
文件: agent-workflows/run\_[demo.py](http://demo.py)

6 步端到端 pipeline: Step 1 - 模型就绪检查 -> model.onnx (OK) Step 2-4 - EZKL 全流程 -> proof + Verifier.sol (SKIP, 已存在) Step 5 - 部署检查 -> 合约已部署 (SKIP) Step 6 - 链上验证 -> verifyProof returns TRUE

引擎选择:

-   Hermes Agent: 当前环境，免费，tool calling
    
-   GLM-4.7-Flash: [Z.AI](http://Z.AI) 免费模型，接入代码已写好
    
-   GLM-5.1: 付费，[Z.AI](http://Z.AI) 赛道旗舰
    

### **3\. GLM API 测试**

-   GLM\_API\_KEY 已过期（401），但不影响 demo
...
-   Agent 编排脚本 (run\_[demo.py](http://demo.py))
...
**项目**: ZKML Pipeline Agent | 赛道: [Z.AI](http://Z.AI) Long-Horizon Task | Week 4 Day 4

**今日完成**

链上 verifyProof 调试 本地验证: ezkl.verify() -> True (proof 有效) ABI 编码: eth\_abi.encode calldata -> cast call 结果: revert 排错链路: bytecode 对比 (forge inspect vs cast code -> EXACT MATCH) 重新生成 proof -> 本地验证 True，链上仍 revert

根因定位 Day 2 生成 proof，Day 3 单独跑 create\_evm\_verifier 如果中间 setup 被重跑，vk.key 变化 -> Verifier.sol 内 VK 与 proof 不匹配 解法: 删 pk.key/vk.key/proof.json/Verifier.sol，一键跑 ezkl\_[pipeline.py](http://pipeline.py) 连续闭环

验证方法沉淀

| 需求 | 命令 |
| --- | --- |
| 本地验证 | ezkl.verify(proof_path=, settings_path=, vk_path=, srs_path=) |
| 合约 bytecode | forge inspect Halo2Verifier deployedBytecode --via-ir |
| 链上 bytecode | cast code --rpc-url <URL> <addr> |
| 调用 trace | cast call --trace <addr> <calldata> |

踩坑记录 ezkl.verify API: 23.0.5 必须 keyword args + 传 srs\_path，位置参数报 "expected value at line 1 column 1" forge inspect bytecode vs deployedBytecode: 前者含 constructor，后者才是链上 runtime code Halo2Verifier gas: ~150k (revert) / ~270k (完整)

仍未解决 链上 verifyProof: 需重新完整闭环 GLM-5.1 API 接入 Pipeline 配置 + 端到端
...
EVM Verifier 生成（突破 Windows 阻塞） ezkl 23.0.5 `create_evm_verifier` 在 Win 下有 tokio async bug: "no running event loop" 解决: 用 `asyncio.run(create_evm_verifier(...))` 绕过，生成 Verifier.sol (1426行/72KB) + ABI

Foundry 编译 + 链上部署 编译坑: Verifier.sol 报 "Stack too deep"，foundry.toml 加 `via_ir = true` 解决 Deploy.s.sol: `vm.startBroadcast()` + `--private-key 0x$PRIVATE_KEY` 绕过 envUint 要求 hex prefix 合约成功部署到 Sepolia

部署信息 合约地址: `0xE99Fb0F146DcBCAd66589C7BF9963B785EE5D495` 部署交易: `0xeecc08bf26fc1ede7eb379024161560eb321bb0a363a759ef0c82f4c29420fe7` Gas 消耗: ~2.26M Sepolia ETH

踩坑记录 | ezkl async/no event loop | asyncio.run() | | Stack too deep | foundry.toml via\_ir = true | | vm.envUint hex prefix | --private-key CLI flag | | Etherscan 自动验证 | 网络不通，待手动 |

Git push 成功（SSH 认证问题已修复）

下一步 (Day 4) 链上 verifyProof: 用 proof.json + instances 调合约验证 GLM-5.1 API 接入 + function calling 端到端: 一句话 -> 全流程自动化
...
**项目**: ZKML Pipeline Agent | 赛道: [Z.AI](http://Z.AI) Long-Horizon Task | Week 4 Day 1-2

**今日完成**

模型训练 + ONNX 导出 线性回归 (5特征, 归一化), R2=1.0 坑: skl2onnx 的 LinearRegressor 算子 EZKL 不认 解决: 手动构建 ONNX 图用 Gemm 算子 model.onnx: 1节点, 291字节

EZKL 全链路跑通 ezkl 23.0.5 pip install 一步到位 (Windows 预编译包) gen\_settings -> calibrate -> compile -> gen\_srs -> setup -> gen\_witness -> prove -> verify Proof: 18160 bytes, 本地验证通过

API 适配踩坑 (ezkl 23.0.5 vs 旧版 API): calibrate\_settings/gen\_witness 参数从 dict 改为文件路径 prove 不再需要 settings 和 proof\_type 参数 gen\_srs 返回 None (不能用 assert 判) create\_evm\_verifier: Windows async 线程 bug (pyo3 panic), 需 Colab 完成

数据归一化的坑: 原始 make\_regression 数据范围太大, calibration 报分解错误 解决: StandardScaler 归一化到 ~\[-1, 1\]

Git SSH 认证: 机上有两把 key (id\_ed25519 chichuzxy / id\_rsa wqq1461394927-arch) id\_ed25519 不在 GitHub Chichuzxy 账号上, push 仍卡住
...
| ZK Proof 生成 | done |
| 本地验证 | done |
| EVM Verifier 合约 | blocked |
| GLM-5.1 API 接入 | pending |

**仍未解决** EVM Verifier: ezkl Win bug -> Colab GLM-5.1 API: 旧 Key 泄露作废 -> 需新 Key Git push: SSH 认证问题 Sepolia 测试币未领
...
主方向: ZKML 推理验证 | 赛道: [Z.AI](http://Z.AI) Long-Horizon Task

今日内容：项目全程踩坑复盘，15 个问题分五类。工具链 8 个、安全 1 个、项目管理 3 个、AI 边界 3 个。仍有 4 个未解决：EZKL 兼容性、GLM API 联通、Sepolia 测试币、.env。
